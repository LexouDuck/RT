...
int					opencl_init_kernels(void)
{
	...
	g_rt.ocl.kernels[0] = clCreateKernel(g_rt.ocl.program, RT_CL_KERNEL_0, &error);
	if (error < 0)
		return (debug_perror("opencl_init:"
							" could not init kernel "RT_CL_KERNEL_0));
	g_rt.ocl.kernels[1] = clCreateKernel(g_rt.ocl.program, RT_CL_KERNEL_1, &error);
	if (error < 0)
		return (debug_perror("opencl_init:"
							" could not init kernel "RT_CL_KERNEL_1));
	g_rt.ocl.kernels[2] = clCreateKernel(g_rt.ocl.program, RT_CL_KERNEL_2, &error);
	if (error < 0)
		return (debug_perror("opencl_init:"
...
int					render_launch_kernel0_build_scene(void)
{
	...
	if ((error = clEnqueueWriteBuffer(g_rt.ocl.cmd_queue, g_rt.ocl.gpu_buf.scene,
	...
	if ((error = clEnqueueNDRangeKernel(g_rt.ocl.cmd_queue, g_rt.ocl.kernels[0], 1,
