...
void			ui_set_result_pixels(t_u8 const *chr, t_u8 **result_pixels, t_u16 tilecount)
{
	t_u16			index;
	t_u8			pixel;
	t_u16			tile;
	...
	while (++tile < tilecount)
	{
		...
		while (++y < TILE)
		{
			while (++x < TILE)
			{
				pixel = 0;
				pixel |= 1 &
				(chr[tile * CHR_BYTES_PER_TILE + 8 + y] >> (7 - x));
				pixel <<= 1;
				pixel |= 1 &
				(chr[tile * CHR_BYTES_PER_TILE + 0 + y] >> (7 - x));
...
*/
	ui_set_result_pixels(chr, &result_pixels, tilecount);
