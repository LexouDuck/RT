...
void		camera_pan(t_camera *camera, float x, float y)
{
	...
	tmp = (cl_float3){{ -camera->pos.x, -camera->pos.y, -camera->pos.z }};
...
void		camera_update(t_camera *camera)
{
	...
	cos = cosf(lon);
	sin = sinf(lon);
	camera->pos.x = camera->zoom * cos * camera->pos.x;
	camera->pos.y = camera->zoom * sin;
	camera->pos.z = camera->zoom * cos * camera->pos.z;
}
